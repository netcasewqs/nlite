...
namespace NLite.DynamicProxy
{
    ...
    public sealed class ProxyFactory
    {
...
#endif
        private static IProxyCache _cache = new ProxyCache();
        private static IProxyMethodBuilder _proxyMethodBuilder = new DefaultyProxyMethodBuilder();
        ...
        private static Type CheckArguments(Type instanceType, Type[] baseInterfaces)
        {
            if (instanceType == null)
            {
                if (baseInterfaces == null || baseInterfaces.Length == 0)
                {
                    throw new ArgumentNullException("instanceType and baseInterfaces");
                }

                instanceType = baseInterfaces[0];
        ...
        public static Type CreateProxyType(Type baseType, params Type[] baseInterfaces)
        {
            baseType = CheckArguments(baseType, baseInterfaces);

            // Reuse the previous results, if possible
            if (_cache.Contains(baseType, baseInterfaces))
        ...
        private static Type CreateUncachedProxyType(Type[] baseInterfaces, Type baseType)
        {
